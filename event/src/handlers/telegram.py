...
class FoxRoseHandler:
    def __init__(self, state_manager, bot_token):
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f"Start command received: {update.effective_user.username} ({update.effective_user.id})")
    ...
    async def handle_keyboard_clicks(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # User picked a duration
            ...
            await update.message.reply_text(
                ...
                reply_markup=self.build_persistent_keyboard(user) # Bring back main menu
            )
            return
